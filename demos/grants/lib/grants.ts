...
import { amountOpportunitys, OppHits, RestDetail } from "./type";
...
export async function fetchOpportunitys(page: number) {
  ...
  const { data }: any = await axios.post(url.opportunity, dataReq, {
    headers: {
      "data-type": "json",
...
export async function fetchDetail(id: number) {
  ...
  const { data }: any = await axios.post(url.detail, dataReq, {
    headers: {
      "data-type": "json",
