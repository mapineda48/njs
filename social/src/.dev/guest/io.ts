import * as guest from "../../socket/guest";
import { event, MIGUEL } from "../../socket/type";
...
export const mockMiguel = {
  ...
  addMessage(data: string) {
    const message: Message = { writeBy: MIGUEL, data };

    server.emit(event.addMessage, message);
  },
};

guest.opt.io = function mockIO() {
  const id = "12345";
  ...
  return {
    ...
    emit(e: string, ...args: any[]) {
      ...
      if (e === event.addMessage) {
        const message: Message = { writeBy: id, data };
