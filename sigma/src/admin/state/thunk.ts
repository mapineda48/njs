...
import { amountResults, Person, Select, Record, Update } from "shared";
import type { Admin } from ".";

export async function fetchColombia(admin: Admin) {
  admin.loading();

  try {
    const colombia = await http.fetchColombia();

    admin.setColombia(colombia);
  } catch (error) {
    admin.confirm({
      error: true,
      reload: true,
      message: error.message,
      onConfirm: () => admin.confirm(null),
    });
  } finally {
    admin.loading(false);
  }
...
export async function fetchPersons(admin: Admin, query: Select) {
  ...
  try {
    const results = await http.fetchPerson(query);
    if (!results.length) {
...
export async function fetchPersonWithScroll(admin: Admin) {
  ...
  try {
    ...
    const results = await http.fetchPerson(nextQuery);
