...
import { route as path, client as custom, IApi, AgapeHeader } from "./client";
import { IApi as IAgape } from "./agape";
import { IApi as IShop } from "./shop";
...
export function initClient() {
  ...
  onSignIn.add((app, token) => {
    ...
    client.model = initModels(session);
  });
  ...
  onExit.add((app) => {
    ...
    if (!(app === AgapeHeader) || !client.model) {
      return;
    }
...
export async function sigin(credential: Authentication) {
  const { data: token } = await create().post<string>(authenticate, [
    credential,
  ]);
  const [app] = Object.keys(credential);
 ...
 * Types
 */
interface IClient extends Omit<IApi, "agape" | "shop"> {
  agape?: IAgape;
  shop?: IShop;
